    ...
    while not no_more_ops:    # iterate until the end of each process is found
        ...
        for pro in Process.processes.values():
            ...
            if pro.operation_counter < len(pro.operations):
                ...
                print("starrt of loop op", op.operation_type, op.host_process,  "time", pro.logical_time, "pro.op_counter", pro.operation_counter)
                # Process.held status
                if pro.status == Process.held:
                    if not isinstance(op, Mutex):
                        ...
                        if op.operation_type in ["send","recv", "print"]:
                    ...
                    # work off mtx msgs
                        ...
                        if pro.operations[pro.operation_counter].operation_type in ["mtx_req_recv", "mtx_req_grant_send", "mtx_req_grant_recv"]:
                            ...
                            assert pro.operations[pro.operation_counter].operation_type == "mtx_req_send"
                            recv_mtx_pro  = Process.processes[pro.operations[pro.operation_counter].target_process]
                            queue_mtx_req_recv(pro, recv_mtx_pro)
                print("end of loop op", op.operation_type, op.host_process,  "time", pro.logical_time, "pro.op_counter", pro.operation_counter)
...
def run_mtx_operation(host_pro):
    ...
    if op.operation_type == "mtx_req_recv":
        ...
        host_pro.logical_time = max(host_pro.logical_time, op.logical_time + 1) 
...
def run_basic_operation(host_pro):
    ...
    if host_pro.status == Process.held and not isinstance(op, Mutex):
        host_pro.status = Process.released
        ...
        if op.operation_type == "send":
            ...
            if (op.target_process, op.content) in host_pro.received_messages:
                if len(host_pro.received_messages[(op.target_process, op.content)]) > 0:
                    ...
                    print("host timestamp", host_pro.logical_time, "sent timestamp", min(host_pro.received_messages[(op.target_process, op.content)]))
                    op.logical_time = timestamp
                    host_pro.logical_time = timestamp
...
def queue_mtx_req_recv(host_pro, target_pro):
    ...
    if target_pro.status == Process.released:
        target_pro.operations.insert(target_pro.operation_counter, Operation(operation_type = "mtx_req_recv", host_process = target_pro.name, content = "mtx_req_recv", logical_time = host_pro.mtx_req_send_time, mutex = False, target_process = host_pro.name))
    ...
    # find the end of the mutex block and mutex request queue and add the mtx_req_recv message to the end of it
        ...
        target_pro.operations.insert(target_pro.operation_counter + mutex_offset, Operation(operation_type = "mtx_req_recv", host_process = recv_target_promtx_pro.name, content = "mtx_req_recv", logical_time = host_pro.mtx_req_send_time, mutex = False, target_process = host_pro.name))
        ...
        if target_pro.mtx_req_send_time < host_pro.mtx_req_send_time:
            ...
            while isinstance((target_pro.operations[target_pro.operation_counter + mutex_offset]), Mutex) or target_pro.operations[target_pro.operation_counter + mutex_offset].operation_type in ["mtx_req_recv",  "mtx_req_send", "mtx_req_grant"]:
                mutex_offset += 1
            target_pro.operations.insert(target_pro.operation_counter + mutex_offset, Operation(operation_type = "mtx_req_recv", host_process = target_pro.name, content = "mtx_req_recv", logical_time = host_pro.mtx_req_send_time, mutex = False, target_process = host_pro.name))
